...
public class RegisterController {
    ...
    private void registerUser(User user, String userType) {
        new Thread(() -> {
            try {
                // 构建注册请求
                String request = buildRegisterRequest(user);
                String response = SocketClient.sendRequest(request);
                
                // 解析响应
                String result = parseRegisterResponse(response);
                Platform.runLater(() -> {
     ...
     * 处理注册结果
     */
    private void handleRegisterResult(String result, String userType) {
        ...
        } else if ("STUDENT_ID_EXISTS".equals(result)) {
            showError("注册失败，学号已存在");
            studentIdField.requestFocus();
        } else if ("TEACHER_ID_EXISTS".equals(result)) {
            showError("注册失败，教师编号已存在");
            teacherIdField.requestFocus();
        } else if ("ADMIN_ID_EXISTS".equals(result)) {
            showError("注册失败，管理员工号已存在");
            adminIdField.requestFocus();
        } else if ("VALIDATION_FAILED".equals(result)) {
     ...
     * 构建注册请求
     */
    private String buildRegisterRequest(User user) {
        StringBuilder request = new StringBuilder("REGISTER:");
        request.append(user.getUsername()).append(":");
        request.append(user.getPassword()).append(":");
        request.append(user.getRole()).append(":");
        ...
        if (user instanceof Student) {
            ...
            request.append(teacher.getTeacherName()).append(":");
            request.append(teacher.getSex()).append(":");
            request.append(teacher.getTechnical()).append(":");
            request.append(teacher.getDepartmentId() != null ? teacher.getDepartmentId() : "");
        } else if (user instanceof Admin) {
            Admin admin = (Admin) user;
            request.append(admin.getAdminId()).append(":");
            request.append(admin.getAdminName());
        }
    ...
    private String parseRegisterResponse(String response) {
        ...
        if (response.startsWith("SUCCESS:REGISTER:")) {
            return "SUCCESS";
        } else if (response.startsWith("ERROR:")) {
            String errorMsg = response.substring("ERROR:".length());
            if (errorMsg.contains("用户名已存在")) {
                return "USERNAME_EXISTS";
            } else if (errorMsg.contains("学号已存在")) {
                return "STUDENT_ID_EXISTS";
            } else if (errorMsg.contains("教师编号已存在")) {
                return "TEACHER_ID_EXISTS";
            } else if (errorMsg.contains("管理员工号已存在")) {
                return "ADMIN_ID_EXISTS";
            } else if (errorMsg.contains("数据验证失败")) {
                return "VALIDATION_FAILED";
            } else if (errorMsg.contains("数据库错误")) {
