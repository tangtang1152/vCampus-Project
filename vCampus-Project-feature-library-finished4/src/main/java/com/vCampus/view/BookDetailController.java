...
public class BookDetailController extends BaseController {
    ...
    public void setBook(Book book) {
        ...
        // 借阅统计（通过 SocketClient 获取）
        try {
            // 暂时使用本地计算，实际应该通过服务器获取统计信息
    ...
    private void loadRecommendations() {
        ...
        try {
            // 按作者推荐（通过 SocketClient 获取）
            String authorRequest = "SEARCH_BOOKS:" + currentBook.getAuthor() + ":全部:默认(最新):1:10";
            String authorResponse = SocketClient.sendRequest(authorRequest);
            List<Book> authorBooks = parseBooksFromResponse(authorResponse);
            
            // 按分类推荐（通过 SocketClient 获取）
            String categoryRequest = "SEARCH_BOOKS:" + currentBook.getCategory() + ":全部:默认(最新):1:10";
            String categoryResponse = SocketClient.sendRequest(categoryRequest);
            List<Book> categoryBooks = parseBooksFromResponse(categoryResponse);
