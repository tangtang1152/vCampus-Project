...
import org.springframework.beans.factory.annotation.Autowired; // <-- 添加
import org.springframework.stereotype.Service; // <-- 添加
...
extends AbstractBaseServiceImpl<User, Integer> implements IUserService {
    ...
    protected boolean doDelete(Integer userId, Connection conn) throws Exception {
        ...
        boolean subRolesDeleted = true;
        java.util.Set<String> roles = user.getRoleSet();
        
        if (roles.contains("STUDENT")) {
            Student s = studentDao.findByUserId(userId, conn);
            if (s != null) {
            	if (!studentDao.delete(s.getStudentId(), conn)) {
            		System.err.println("删除学生记录失败，学号: " + s.getStudentId());
            		subRolesDeleted = false;
            		}
            }
        }
        
        if (roles.contains("TEACHER")) {
            Teacher t = teacherDao.findByUserId(userId, conn);
            if (t != null) {
            	if (!teacherDao.delete(t.getTeacherId(), conn)) {
            		System.err.println("删除教师记录失败，教师ID: " + t.getTeacherId());
            		subRolesDeleted = false;
            	}
            }
        }
        ...
        if (roles.contains("ADMIN")) {
            Admin a = adminDao.findByUserId(userId, conn);
            if (a != null) {
            	if (!adminDao.delete(a.getAdminId(), conn)) {
            		System.err.println("删除管理员记录失败，工号: " + a.getAdminId());
            		subRolesDeleted = false;
            	}
            }
        }
        if (!userDao.delete(userId, conn)) {
            System.err.println("删除用户主记录失败，用户ID: " + userId);
            return false;
        }

        return subRolesDeleted;
    }
     ...
     * 注册教师账户（事务内部使用）
     */
    private RegisterResult registerTeacher(Teacher teacher, Connection conn) {
        try {
     ...
     * 注册管理员账户（事务内部使用）
     */
    private RegisterResult registerAdmin(Admin admin, Connection conn) {
        try {
     ...
     * 注册学生账户（事务内部使用）
     */
    private RegisterResult registerStudent(Student student, Connection conn) {
        try {
