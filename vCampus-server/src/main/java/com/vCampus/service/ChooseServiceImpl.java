...
public class ChooseServiceImpl extends AbstractBaseServiceImpl<Choose, String> implements IChooseService {
    ...
    // 实现抽象方法（这些方法保持返回 boolean，因为它们继承自 AbstractBaseServiceImpl）
    @Override
    ...
    // *** 选课方法修改为返回 ServiceResult ***
    @Override
    public ServiceResult chooseSubject(String studentId, String subjectId) {
        // 正常学生选课，不做“选课有效期”检查（按你的需求），仍保留容量/重复/时间冲突检查
        return doChooseSubject(studentId, subjectId, false, true); // 不忽略时间冲突，忽略选课有效期
    }

    @Override
    public ServiceResult adminAssistChooseSubject(String studentId, String subjectId, boolean ignoreTimeConflict) {
     ...
     * @return ServiceResult 选课结果，包含成功/失败和具体消息
     */
    private ServiceResult doChooseSubject(String studentId, String subjectId, boolean ignoreTimeConflict, boolean ignoreCourseSelectionPeriod) {
        try {
  ...
  return ServiceResult.fail("选课失败：学生不存在，请检查学号。"); 
                }
  ...
  return ServiceResult.fail("选课失败：课程不存在，请检查课程ID。");
                }
                // 课程时间信息若不完整，记录警告但继续允许选课
  ...
  return ServiceResult.fail("选课失败：已超过选课时间范围。");
                }
  ...
  // 扣减名额后失败，需要回滚名额
  subjectDao.increaseSlot(subjectId, conn); 
  return ServiceResult.fail("选课失败：与已有课程时间冲突。");
                }
  ...
  // 插入失败，回滚名额
  subjectDao.increaseSlot(subjectId, conn);
  return ServiceResult.fail("选课失败：创建选课记录失败。");
                }
            ...
            return ServiceResult.fail("选课操作失败：系统异常，请联系管理员。");
        }
    }


    @Override
    public ServiceResult dropSubject(String selectid) { // 已修改返回类型
        try {
            return TransactionManager.executeInTransaction(conn -> {
                // 1. 验证选课记录存在
  ...
  return ServiceResult.fail("退选失败：选课记录不存在。");
                }
                ...
                //     return ServiceResult.fail("退选失败：已超过退选时间。");
                // }
  ...
  System.err.println("退选成功但回补名额失败：课程ID " + choose.getSubjectId() + ", 错误: " + ignore.getMessage());
  // 即使回补失败，退选操作本身也算成功，但可以考虑在这里返回一个警告或部分成功的 ServiceResult
  }
  System.out.println("退选成功，选课ID: " + selectid);
  return ServiceResult.ok("退选成功！");
                }
                return ServiceResult.fail("退选失败：删除选课记录失败。");
            });
        } catch (Exception e) {
            handleException("退选操作失败", e);
            return ServiceResult.fail("退选操作失败：系统异常，请联系管理员。");
        }
    }
