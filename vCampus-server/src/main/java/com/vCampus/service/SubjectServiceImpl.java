...
import com.vCampus.dao.IChooseDao; // <-- 确保这里有导入
...
public class SubjectServiceImpl extends AbstractBaseServiceImpl<Subject, String> implements ISubjectService {
    ...
    @Autowired private ISubjectDao subjectDao;
    @Autowired private IChooseDao chooseDao;
    ...
    protected boolean doDelete(String subjectId, Connection conn) throws Exception {
        // 1. 删除与该课程关联的所有选课记录
        List<Choose> choosesToDelete = chooseDao.findBySubjectId(subjectId, conn);
        for (Choose choose : choosesToDelete) {
            chooseDao.delete(choose.getSelectid(), conn);
        }
        // 2. 然后删除课程本身
